...
Function get_k1_position_filter, input_data, k
  ...
  for i=0, dimension[0]-1 do begin    
    for j=0, dimension[1]-1 do begin    
      ...
      ;斜率值
      deriv_data = deriv(orgin_data)
      ;原始值最大值及其位置
      max_value = MAX(orgin_data, max_pos, /NAN)
      ;获取原始数据最大值之前的斜率数据（包括最大值）
      data_0 = make_array(max_pos+1)
      for k = 0L, max_pos do begin
        data_0[k] = deriv_data[k]
      endfor
      ;
      k1_position = WHERE(data_0 GT k, count)
      ;如果没有大于k1的输出0
      if count eq 0 then k1_position = 0
      ;
      output_data[i, j] = k1_position[0]
...
Pro get_k1_position_via_all, input_file_id, output_file, k
...
Pro get_k1_position_via_tile, input_file_id, output_file, k
  ...
  for i=0, tiles_num-1 do begin
    ...
    ;##############################Tile处理部分############################################################
    output_data = get_k1_position_filter(data, k)
    ;#####################################################################################################    
...
Pro get_k1_position
  ...
  base = WIDGET_AUTO_BASE(TITLE='请输入参数K1')
  wp = WIDGET_PARAM(base, DT=4, FLOOR=0, PROMPT='k1：最大值前斜率大于K1的第一个点位', UVALUE='param1', /AUTO_MANAGE)
  ...
  if (file_size ge max_file_size) then begin
    ...
    print, '分块处理'
    get_k1_position_via_tile, fid, output_file, k1
    
  endif else begin
    
    print, '整体处理'
    get_k1_position_via_all, fid, output_file, k1
