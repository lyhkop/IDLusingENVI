...
Function get_k3_position_filter, input_data, k
...
;      endfor
      ;获取原始数据最大值之后的数据(包括最大值)
      data_1 = make_array(n_elements(orgin_data)-(max_pos))
      for k=max_pos, n_elements(orgin_data)-1 do begin
        data_1[k-max_pos] = orgin_data[k]
      endfor
      ;
      k3_position = WHERE(data_1 GT k, count)
      ;如果没有大于k2的输出0
      if count eq 0 then k2_position = 0
      ;
      output_data[i, j] = k3_position[-1]
...
Pro get_k3_position_via_all, input_file_id, output_file, k
  ...
  for i=0, ns-1 do begin
    for j=0, nl-1 do begin
      ...
      k3_position = WHERE(data_1 GT k, count)
      ;如果没有大于k2的输出0
      if count eq 0 then k3_position = 0
      ;
      output_data[i, j] = k3_position[-1]
...
Pro get_k3_position_via_tile, input_file_id, output_file, k
  ...
  for i=0, tiles_num-1 do begin
    ...
    ;##############################Tile处理部分############################################################
    output_data = get_k3_position_filter(data, k)
    ;#####################################################################################################    
...
Pro get_k3_position
  ...
  base = WIDGET_AUTO_BASE(TITLE='请输入参数K3')
  wp = WIDGET_PARAM(base, DT=4, FLOOR=0, PROMPT='k3：最大值后数值大于K3的最后一个点位', UVALUE='param1', /AUTO_MANAGE)
  ...
  if (file_size ge max_file_size) then begin
    ...
    print, '分块处理'
    get_k3_position_via_tile, fid, output_file, k2
    
  endif else begin
    
    print, '整体处理'
    get_k3_position_via_all, fid, output_file, k2
