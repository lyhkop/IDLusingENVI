Pro Cal_evi, input_file_fid, pos, output_file_name
...
;  exp='(float(b3)-float(b2))/(float(b3)+float(b2))'
  ...
  dims=dims, out_bname='evi', out_name=output_file_name, $
  exp='2.5*(float(b3)/1000 - float(b2)/1000)/(float(b3)/1000 + 6*float(b2)/1000 - 7.5*float(b1)/1000 + 1)'
  
End


Pro Cal_evi_batch, ev
  ...
  for i=0, count-1 do begin
    ...
    output_file_name = output_files_path + file_basename(input_files[i], '.ld3') + '_evi.ld3'
    
    cal_evi, fid, pos, output_file_name
